...
export default function Rolador() {
  ...
  // const [sucessosFinais, setSucessosFinais] = useState(0);
...
//     const response = await axios.get(`${env.apiURI}/`);
//     setRollsList(response.data);
//     console.log(response.data);
//     console.log(rollsList);
//   }

  function addToList(e: ChangeEvent<HTMLFormElement>) {
    e.preventDefault();
          ...
          {rollsList.map((rolls) => (
            <tbody>
              <tr key={rolls.id}>
                ...
                <td>{rolls.roll.filter(item => item >= dificuldade).length} + {rolls.success}</td>
              </tr>
            </tbody>
          ))}
        </Table>
